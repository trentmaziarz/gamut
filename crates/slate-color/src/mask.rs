...
//! image. mask.wgsl and the masked develop pass in slate-gpu mirror this
...
use slate_core::{Adjustments, CropRect, EXPOSURE_LIMIT, PhotoEdit, SLIDER_LIMIT, Wheel};
...
pub fn active_masks(edit: &PhotoEdit) -> Vec<(usize, Mask)> {
    slate_core::mask::sanitised(&edit.masks)
        .into_iter()
        .enumerate()
...
mod tests {
    use super::*;
    use crate::curve;
    use slate_core::look::Curve;
    use slate_core::mask::Component;
    ...
    fn a_mask_curve_runs_after_the_global_curves() {
        let global = busy_global().look.curves;
        let mask = slate_core::ToneCurves {
            red: Curve {
