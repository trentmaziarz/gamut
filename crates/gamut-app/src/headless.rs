...
use gamut_core::{Crop, CropRect, Sidecar};
use gamut_gpu::Headless;
use gamut_media::export::ExportError;
use gamut_media::{Photo, PhotoError, VideoError, open_photo};
...
pub fn mask_named(sidecar: &Sidecar, name: &str) -> Result<usize, HeadlessError> {
    ...
    gamut_core::mask::find(masks, name).ok_or_else(|| {
        let name = name.trim();
