...
use slate_core::{Crop, CropAspect, CropRect, Project, Track};
use slate_gpu::{Develop, Headless, PendingReadback, Readback};
use slate_media::audio::AudioSource;
use slate_media::ffmpeg::Rational;
use slate_media::ffmpeg::frame;
use slate_media::video_export::{AUDIO_RATE, ReelWriter};
use slate_media::{DecodedFrame, FramePlanes, VideoError, VideoSource};
...
pub fn export(project: &Project, dir: &Path, out: &Path) -> Result<f64, HeadlessError> {
    ...
    let decode_result = decoder.join().unwrap_or_else(|_| {
        Err(VideoError::Encode {
            ...
            source: slate_media::ffmpeg::Error::Unknown,
        })
    });
...
fn decode(
    ...
    let mut layout: Option<(slate_media::ffmpeg::format::Pixel, u32, u32)> = None;
    for i in 0..frames {
