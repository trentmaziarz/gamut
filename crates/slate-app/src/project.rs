...
use slate_core::{Clip, Project, Track};
...
pub fn for_video(video: &Path, info: &MediaInfo) -> LoadedProject {
    ...
    project.crop =
        slate_core::Crop::fitted(slate_core::CropAspect::Story9x16, info.width, info.height);
...
pub fn rebase(project: &mut Project, old_dir: &Path, new_dir: &Path) {
    for media in &mut project.media {
        let absolute = slate_core::project::resolve_path(old_dir, &media.path);
        media.path = slate_core::project::relative_path(new_dir, &absolute);
    }
}
